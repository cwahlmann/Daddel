...
public class TutorialMondauto08Interaktion extends Daddel {
	...
	private void bewegeMarsipolinchen(Entity marsipolinchen) {
		...
		frissQuecksilberUndLegeEier(marsipolinchen);		
	}
	...
	// was macht das Mondauto mit Eiern und Eierschalen?

	private void fahreEierKaputt(Entity mondauto) {
		// und jetzt mal gucken, ob da was liegt
		if (mondauto.checkId(id -> id == MARSEI_ID)) {
			// ein Marsei wird durch eine Eierschale ersetzt
			mondauto.drop(MARSEIERSCHALE_ID);
		} else if (mondauto.checkId(id -> id == MARSEIERSCHALE_ID)) {
			// eine Eierschale durch Quecksilber
			mondauto.drop(QUECKSILBER_ID);
		}
	}

	// was macht das Marsipolinchen?

	private void frissQuecksilberUndLegeEier(Entity marsipolinchen) {
		// und jetzt mal gucken, ob da was liegt
		if (marsipolinchen.checkId(id -> id == QUECKSILBER_ID)) {
			// Quecksilber wird geschluckt
			marsipolinchen.take();
		} else if (marsipolinchen.checkId(id -> id == TileMap.NO_ID)) {
			// wenn noch nichts da liegt, zufällig ein Ei legen
			if (random.nextDouble() > 0.9) {
				marsipolinchen.drop(MARSEI_ID);
			}
		}
	}
